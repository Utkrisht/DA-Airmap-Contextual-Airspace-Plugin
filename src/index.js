...
export default class ContextualAirspacePlugin {
  ...
  getJurisdictionsFromMap = () => {
    ...
    let filteredLayers = null

    if (lastIndex == -1 && layers.length > 0) {
      filteredLayers = layers
    } else {
      filteredLayers = layers.slice(lastIndex)
    }

    let jurisdictions = filteredLayers
  ...
  updateRulesets = (preferredRulesets, overrideRulesets, enableRecommendedRulesets) => {
    ...
    const defaultSelectedRulesets = getDefaultSelectedRulesets(
      parsedJurisdictionRulesets,
      preferredRulesets,
      null,
      this.options.enableRecommendedRulesets
    )
  ...
  receiveJurisdictions = preferredRulesets => {
    ...
    // Gathers the default selected rulesets: required, pick1 defaults, and optional if selected by user previously
    let defaultSelectedRulesets = getDefaultSelectedRulesets(
      parsedJurisdictionRulesets,
      preferredRulesets,
      null,
      this.options.enableRecommendedRulesets
   ...
   */
  handleMapClick = data => {
    const features = data.target
      .queryRenderedFeatures(data.point)
